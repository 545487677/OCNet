...
class FinetuneMSELoss(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if not self.training:
            if hasattr(self.task, 'mean') and hasattr(self.task, 'std') and self.task.mean and self.task.std:
            ...
            logging_output = {
                ...
                "conf_size": getattr(self.args, "conf_size", 1),
    ...
    def reduce_metrics(logging_outputs, split="valid") -> None:
        ...
        if "valid" in split or "test" in split:
            ...
            for task_idx in range(num_task):
                task_mae = np.abs(y_pred[:, task_idx] - y_true[:, task_idx]).mean()
                metrics.log_scalar(f"{split}_task{task_idx}_mae", task_mae, sample_size, round=4)
