...
@register_loss("finetune_mse_infer")
class FinetuneMSEInferLoss(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if  hasattr(self.args, "mode") and self.args.mode == "infer":
            logging_output = {
                "rep": reg_output,
                "smi_name": sample["smi_name"],
            }
            return None, None, logging_output
        ...
        if not self.training:
            ...
            logging_output = {
                ...
                "target": sample["target"]["finetune_target"].view(-1, self.args.num_classes).data if sample["target"]["finetune_target"] is not None else None,
                "smi_name": sample["smi_name"],
                "sample_size": sample_size,
                "num_task": self.args.num_classes,
                "conf_size": self.args.conf_size if hasattr(self.args, "conf_size") else 1,
...
@register_loss("finetune_mae_infer")
class FinetuneMAEInferLoss(FinetuneMSEInferLoss):
    def __init__(self, task):
        super().__init__(task)
...
@register_loss("finetune_smooth_mae_infer")
class FinetuneSmoothMAEInferLoss(FinetuneMSEInferLoss):
    def __init__(self, task):
        super().__init__(task)
    ...
    def reduce_metrics(logging_outputs, split="valid") -> None:
        ...
        if "valid" in split or "test" in split:
            ...
            for task_idx in range(num_task):
                r2 = r2_score(y_true[:, task_idx], y_pred[:, task_idx])
                metrics.log_scalar(f"{split}_task{task_idx}_r2_score", r2, sample_size, round=4)
...
@register_loss("finetune_mse_pocket_infer")
class FinetuneMSEPocketInferLoss(FinetuneMSEInferLoss):
    def __init__(self, task):
        super().__init__(task)
