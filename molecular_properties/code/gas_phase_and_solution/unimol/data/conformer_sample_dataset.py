...
class ConformerSamplePairDataset(BaseWrapperDataset):
    def __init__(self, dataset, seed, atoms_a, coordinates_a, atoms_b, coordinates_b):
        self.dataset = dataset
        self.seed = seed
    ...
    @lru_cache(maxsize=16)
    def __cached_item__(self, index: int, epoch: int):
        atoms_a = np.array([item[0] for item in self.dataset[index][self.atoms_a]])
        assert len(atoms_a) > 0
        coordinates_a = self.dataset[index][self.coordinates_a][0]
        assert len(atoms_a) == len(coordinates_a)

        atoms_b = np.array([item[0] for item in self.dataset[index][self.atoms_b]])
        assert len(atoms_b) > 0
        coordinates_b = self.dataset[index][self.coordinates_b][0]
        assert len(atoms_b) == len(coordinates_b)

        return {'atoms': atoms_a, 
                'coordinates': coordinates_a.astype(np.float32), 
                'solvent_atoms': atoms_b, 
                'solvent_coordinates': coordinates_b.astype(np.float32)
                }
