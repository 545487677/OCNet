...
weight_path='../../../weight/properties_in_solution/fwhm/checkpoint_best.pt'  # replace to your ckpt path
batch_size=16
task_name='fwhm' # data folder name 
task_num=1
...
torchrun --nproc_per_node=$n_gpu --master_port=$MASTER_PORT ../unimol/infer.py --user-dir ../unimol $data_path --task-name $task_name --valid-subset valid \
       ...
       --classification-head-name 'deep4chem_fwhm_single_tower_v1_feature5' --num-classes $task_num \
       --dict-name $dict_name \
...
# R2 for Column 0: 0.7194333435959337
# MAE for column 0: 9.122739664345506
# (1382, 4)