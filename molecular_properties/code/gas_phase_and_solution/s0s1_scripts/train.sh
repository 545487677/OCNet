...
weight_path="../../../weight/pretrain/checkpoint_best.pt" 
task_name="s0s1"  # molecular property prediction task name 
task_num=1 
...
if [ "$task_name" == "qm7dft" ] || [ "$task_name" == "qm8dft" ] || [ "$task_name" == "qm9dft" ] || [ "$task_name" == "s0s1" ]; then
    ...
    for dropout in "${dropout_values[@]}"; do
      for warmup in "${warmup_values[@]}"; do
        for encoder_layers in "${encoder_layers_values[@]}"; do
          ...
          exp=l${task_name}_lr${lr}_bs${global_batch_size}_dropout${dropout}_warmup${warmup}_layers${encoder_layers}
          log_path=./$task_name
          echo "$exp"
