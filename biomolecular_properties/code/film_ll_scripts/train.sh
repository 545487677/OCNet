...
if [ "$task_name" == "qm7dft" ] || [ "$task_name" == "qm8dft" ] || [ "$task_name" == "qm9dft" ] || [ "$task_name" == "opv" ] || [ "$task_name" == "opv_rdkit_dft" ] || [ "$task_name" == "film_ll" ]; then
	metric="valid_mae"
        ...
        do
            ...
            do
                ...
                torchrun  --nproc_per_node=$n_gpu --master_port=$MASTER_PORT $(which unicore-train) $data_path --user-dir ../unimol --train-subset train --valid-subset valid \
                    ...
                    --task-name $task_name \
                    --dict-name pretrain_dict.txt \
                    --task trans_mix_feature --loss trans_loss --arch trans_features0  \
